...
function extractLinks(html, baseUrl, rootDomain) {
  ...
  var links = [];
  var navLinks = []; // nav/menu links get priority
  $('a[href]').each(function(_, el) {
    try {
      ...
      // Check if this link is in nav/header/menu
      var inNav = $(el).closest('nav, header, .nav, .menu, .header, [class*="nav"], [class*="menu"]').length > 0;
      if (inNav) navLinks.push(resolved);
      else links.push(resolved);
    } catch(e) {}
  });
  // Return nav links first, then other links
  return navLinks.concat(links);
}
...
function parsePage(html, url, status) {
  ...
  var h2s = $('h2').map(function(_, el) { return $(el).text().trim(); }).get().slice(0, 10);
  ...
  dedupedPages.forEach(function(page) {
    try {
      ...
      nodesByPath[pathname] = {
        ...
        hasSchema: (page.schemas && page.schemas.length > 0) || false,
        children: []
      };
    } catch(e) {}
  });
  ...
  Object.keys(nodesByPath).sort().forEach(function(pathname) {
    ...
    if (!nodesByPath[parentPath]) {
      ...
      nodesByPath[parentPath] = {
        url: origin + parentPath, label: parentPath, depth: vParts.length,
        status: 'ok', isOrphan: false, type: 'section',
        pageTitle: '', wordCount: 0, hasSchema: false, virtual: true, children: []
      };
...
async function crawlSite(rootUrl, options) {
  ...
  var visited = new Set();
  // Priority queue: nav links first, then other crawled links, then sitemap URLs
  var priorityQueue = [normalizedRoot]; // Start with homepage
  var regularQueue = [];
  var sitemapQueue = [];

  // Add sitemap URLs sorted by priority (higher priority pages first)
  if (xmlSitemap.found) {
    var sitemapUrls = xmlSitemap.urls.slice();
    // Sort by priority descending, then by URL depth ascending (shorter = more important)
    sitemapUrls.sort(function(a, b) {
      var depthA = (new URL(a.loc).pathname.match(/\//g)||[]).length;
      var depthB = (new URL(b.loc).pathname.match(/\//g)||[]).length;
      ...
      if (crawlSubpages && result.status < 400) {
        ...
        links.forEach(function(link, idx) {
          ...
          if (!visited.has(normLink)) {
            // Nav links (first in array) go to priority queue

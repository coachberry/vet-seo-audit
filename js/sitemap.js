...
function renderSitemap(data) {
  ...
  var issuePages = allPages.filter(function(p) {
    return p && (p.status === 'noindex' || p.status === 'error' || p.status === 'redirect' || p.isOrphan);
  });
  ...
  function renderTree(nodes, depth) {
    ...
    nodes.forEach(function(n) {
      if (!n) return;
      var trueStatus = n.isOrphan && n.status === 'ok' ? 'orphan' : n.status;
      ...
      var statusCls = 'status-' + (trueStatus || 'ok');
      ...
      html += '<div class="sitemap-node" style="padding-left:' + (depth * 16) + 'px">' +
        '<button onclick="openUrl(\'' + safeUrl + '\')" class="btn-ghost btn-sm" ' +
          'style="padding:.1rem .3rem;font-size:.62rem;margin-right:.3rem;flex-shrink:0;background:transparent;border:1px solid var(--border);border-radius:4px;color:var(--accent2);cursor:pointer" ' +
          'title="Open ' + (n.url || '') + '">↗</button>' +
  ...
  var noAnalysisMsg = '<div style="font-size:.82rem;color:var(--muted);font-family:\'DM Mono\',monospace">AI analysis timed out for this section. Run the full SEO Audit for detailed recommendations.</div>';
  ...
  document.getElementById('sitemapOutput').innerHTML =
    ...
    '<div class="score-grid" style="grid-template-columns:repeat(4,1fr)">' +
      ...
      '<div class="score-card"><div class="score-label">Pages w/ Issues</div>' +
        ...
        '<div style="font-size:.65rem;color:var(--muted);margin-top:.2rem">' + allPages.length + ' total pages</div>' +
      '</div>' +
    '</div>' +
      ...
      '<div class="page-card-body open">' +
        (strengths.length ?
          '<ul style="padding-left:1.1rem;font-size:.875rem;line-height:2.1">' +
            strengths.map(function(s) { return '<li class="green">' + s + '</li>'; }).join('') +
          '</ul>' : noAnalysisMsg) +
      '</div>' +
    '</div>' +
    ...
    '<div class="page-card" style="margin-bottom:.75rem">' +
      ...
      '<div class="page-card-body open">' +
        (issues.length ?
          '<ul style="padding-left:1.1rem;font-size:.875rem;line-height:2.1">' +
            issues.map(function(s) { return '<li class="red">' + s + '</li>'; }).join('') +
          '</ul>' : noAnalysisMsg) +
      '</div>' +
    '</div>' +
...
function getDefaultIssue(status, page) {
  ...
  if (status === 'orphan') {
    return 'ORPHAN PAGE — No internal links from any other crawled page point to this URL. ' +
      ...
      'Word count: ' + wc + ' words. Schema: ' + (hasSchema ? 'Present' : 'Missing') + '. ' +
      'Orphaned pages receive no link equity from your site and are harder for Google to discover and prioritize.';
  }
  if (status === 'noindex') return 'NOINDEX — A robots meta tag on this page tells search engines NOT to index it. It will not appear in Google search results regardless of how good the content is.';
  if (status === 'error') return 'HTTP ' + ((page && page.status) || '4xx/5xx') + ' ERROR — This page returned an error. Search engines cannot crawl or index broken pages and will eventually stop trying.';
  if (status === 'redirect') return 'REDIRECT — This URL permanently or temporarily redirects to another page. Links and link equity pointing to this URL are being transferred to the destination.';
...
function getDefaultRecommendation(status) {
  if (status === 'orphan') return 'Add internal links to this page from relevant service pages, the blog index, navigation menus, or related blog posts. If this content is outdated or low quality, consider consolidating it with a stronger page via a 301 redirect, or unpublishing it entirely to focus crawl budget on high-value pages.';
  if (status === 'noindex') return 'Verify this noindex tag is intentional. If this page should appear in Google search results, remove the noindex directive. Check both the HTML meta robots tag and the X-Robots-Tag HTTP header.';
  if (status === 'error') return 'Fix the page or set up a 301 redirect to the most relevant live page. Remove this URL from your XML sitemap and update any internal links pointing to it.';
  if (status === 'redirect') return 'Update all internal links and sitemap entries to point directly to the final destination URL. Redirect chains waste crawl budget and dilute link equity.';

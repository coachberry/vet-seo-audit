...
function renderAudit(data, sortBy, sortDir, minScore, filterKey) {
  ...
  var avgCards = SCORE_KEYS.map(function(sk) {
    ...
    return '<div class="score-card"><div class="score-label">' + sk.label + '</div>' +
  ...
  pages.sort(function(a,b) {
    var va = a.scores&&a.scores[sortBy]||0, vb = b.scores&&b.scores[sortBy]||0;
...
function buildPageCard(page) {
  ...
  var auditSections = AUDIT_KEYS.map(function(ak) {
    return '<div class="audit-section"><div class="audit-section-title">' + ak.label + '</div>' +
      '<div class="audit-section-body">' + (page.audit&&page.audit[ak.key]||'') + '</div></div>';
  }).join('');
  var priorities = page.audit&&page.audit.priorityActions||[];
  var priorityHTML = priorities.length ? '<div class="priority-box"><div class="priority-box-title">⚡ Priority Actions</div>' +
    priorities.map(function(a,i) {
      return '<div class="priority-item"><span class="priority-num">' + (i+1) + '.</span>' +
  ...
  var schemaSection = (schemaFound||schemaMissing) ?
    '<div class="audit-section" style="grid-column:1/-1"><div class="audit-section-title">Schema Found / Missing</div>' +
    '<div class="schema-badges">' + schemaFound + schemaMissing + '</div></div>' : '';
  return '<div class="page-card">' +
    '<div class="page-card-header" onclick="toggleCard(this)">' +
      '<div class="page-meta">' +
        '<div style="display:flex;align-items:center;gap:.5rem">' +
          '<div class="page-url">' + page.url + '</div>' +
          '<button onclick="event.stopPropagation();openUrl(\'' + page.url.replace(/'/g,"\\'") + '\')" class="btn btn-ghost btn-sm" style="padding:.1rem .35rem;font-size:.65rem;flex-shrink:0">↗</button>' +
        '</div>' +
...
function applyAuditFilters() {
  ...
  renderAudit(lastAuditData,
    document.getElementById('sortKey')&&document.getElementById('sortKey').value,
    document.getElementById('sortDir')&&document.getElementById('sortDir').value,
    document.getElementById('minScore')&&parseInt(document.getElementById('minScore').value),
    document.getElementById('filterKey')&&document.getElementById('filterKey').value);
}

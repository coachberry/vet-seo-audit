...
async function callFunction(endpoint, payload) {
  var res = await fetch(FUNCTIONS_BASE_URL + '/' + endpoint, {
    method: 'POST',
...
async function runAudit() {
  ...
  var iv = setLoading('auditOutput', 'CRAWLING & AUDITING', [
  ...
  try {
    var data = await callFunction('auditSite', { url: url, pageLimit: pageLimit, crawlSubpages: crawlSubpages });
    finishLoading(iv);
    lastAuditData = data;
...
function buildPageCard(page) {
  ...
  SCORE_KEYS.slice(0,4).forEach(function(sk) {
    ...
    var short = sk.label.split(' ')[0];
    miniScores += '<span class="mini-score ' + scoreClass(v) + '" style="border-color:' + scoreColor(v) + ';background:' + scoreColor(v) + '15">' + short + ': ' + v + '</span>';
  });

  var flags = '';
  (page.flags || []).forEach(function(f) {
    flags += '<span class="flag flag-' + f.type + '">' + f.label + '</span>';
  });

  var schemaFound = '';
  (page.schemaTypes || []).forEach(function(s) { schemaFound += '<span class="schema-badge">' + s + '</span>'; });
  var schemaMissing = '';
  (page.missingSchema || []).forEach(function(s) { schemaMissing += '<span class="schema-badge missing">Missing: ' + s + '</span>'; });

  var auditSections = '';
  AUDIT_KEYS.forEach(function(ak) {
    auditSections += '<div class="audit-section">' +
      '<div class="audit-section-title">' + ak.label + '</div>' +
      '<div class="audit-section-body">' + (page.audit&&page.audit[ak.key]||'') + '</div>' +
      '</div>';
  });
  ...
  if (priorities.length) {
    ...
    priorities.forEach(function(a, i) {
      priorityHTML += '<div class="priority-item">' +
        ...
        '<div><div class="priority-text">' + (a.action||a) + '</div>' +
        (a.impact ? '<span class="priority-impact impact-' + a.impact.toLowerCase() + '">' + a.impact + ' IMPACT</span>' : '') +
        '</div></div>';
    });
    priorityHTML += '</div>';
  }
  ...
  var schemaSection = (schemaFound || schemaMissing) ?
    '<div class="audit-section" style="grid-column:1/-1"><div class="audit-section-title">Schema Found / Missing</div><div class="schema-badges">' + schemaFound + schemaMissing + '</div></div>' : '';
...
async function runSitemap() {
  ...
  var iv = setLoading('sitemapOutput', 'BUILDING SITEMAP', [
    'Fetching root page...',
  ...
  try {
    var data = await callFunction('buildSitemap', { url: url });
    finishLoading(iv);
...
function renderSitemap(data) {
  ...
  document.getElementById('sitemapOutput').innerHTML =
    ...
    '<div class="page-card" style="margin-bottom:.75rem">' +
      ...
      '</div>' +
      '<div class="page-card-body open"><div class="sitemap-wrapper"><div class="sitemap-tree">' + renderTree(data.sitemapTree||[]) + '</div></div></div>' +
    '</div>' +
    issueCards +
    '<div class="page-card" style="margin-bottom:.75rem">' +
      '<div class="page-card-header expanded" onclick="toggleCard(this)"><span style="font-family:\'DM Mono\',monospace;font-size:.73rem;color:var(--green)">✦ STRENGTHS</span><span class="arrow-icon" style="margin-left:auto">▾</span></div>' +
      '<div class="page-card-body open"><ul style="padding-left:1.1rem;font-size:.86rem;line-height:2.1">' + (data.urlAnalysis&&data.urlAnalysis.strengths||[]).map(function(s){return '<li class="green">'+s+'</li>';}).join('') + '</ul></div>' +
    '</div>' +
    '<div class="page-card" style="margin-bottom:.75rem">' +
      '<div class="page-card-header expanded" onclick="toggleCard(this)"><span style="font-family:\'DM Mono\',monospace;font-size:.73rem;color:var(--red)">⚠ ISSUES</span><span class="arrow-icon" style="margin-left:auto">▾</span></div>' +
      '<div class="page-card-body open"><ul style="padding-left:1.1rem;font-size:.86rem;line-height:2.1">' + (data.urlAnalysis&&data.urlAnalysis.issues||[]).map(function(s){return '<li class="red">'+s+'</li>';}).join('') + '</ul></div>' +
    '</div>' +
    '<div class="page-card">' +
      '<div class="page-card-header expanded" onclick="toggleCard(this)"><span style="font-family:\'DM Mono\',monospace;font-size:.73rem;color:var(--blue)">📊 FULL ANALYSIS</span><span class="arrow-icon" style="margin-left:auto">▾</span></div>' +
      '<div class="page-card-body open"><div class="prose">' + (data.overallReport||'').split('\n\n').map(function(p){return '<p>'+p+'</p>';}).join('') + '</div></div>' +
    '</div>';
}
...
async function runCompare() {
  ...
  var iv = setLoading('compareOutput', 'COMPARING SITES', [
    'Crawling site 1...', 'Crawling site 2...', 'Scoring both sites...', 'Building comparison...'
  ]);
  try {
    var data = await callFunction('compareSites', { url1: u1, url2: u2 });
    finishLoading(iv);
    lastCompareData = data;
    await saveToFirestore('compare', (data.site1&&data.site1.domain||'') + ' vs ' + (data.site2&&data.site2.domain||''), data);
    renderCompare(data);
  } catch(e) {
    finishLoading(iv);
...
function renderCompare(data) {
  ...
  document.getElementById('compareOutput').innerHTML =
    ...
    '<div class="audit-grid" style="align-items:start">' +
      '<div><div class="section-title" style="font-size:1rem">' + (s1.domain||'Site 1') + '</div>' + s1Pages + '</div>' +
      '<div><div class="section-title" style="font-size:1rem">' + (s2.domain||'Site 2') + '</div>' + s2Pages + '</div>' +
    '</div>';
}
